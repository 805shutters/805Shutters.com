...
## Portfolio shade valance and mounting increment

The shade panel now captures mount, top-bracket inside depth (minimum ¾ inch), no/standard/outside-only extended returns, front/back/interior valance choices, and standard/custom front-valance length. Shared validation rejects inside-mount TDBU returns, missing mandatory TDBU front/back valances, waterfall-only interior options on standard shades, and unconfirmed custom lengths. Changing control/drop/mount clears incompatible dependent selections. Source pages 8, 12, 14 and 18 govern these choices; source page 25 states a $51 retail waterfall front-valance charge. No deduction is silently applied where pages 18/20 conflict. Tests: 419 Sundance/rules checks and TypeScript passed.

Remaining internal work is concrete: accessory selectors and source charge breakdown for the represented controls; motor choice compatibility for additional SheerView motors; individual two-on-one measurements and charges; source-backed controls/size/mount/option rules for cellular, Walden, roller/Europanel, Zebra, vertical, horizontal and Glydea; complete source applicability and live comparisons. Current captured identity exceptions and account pricing authority are separate external work.

## Accessory and option retail evidence

The saved panels now expose control-specific quantities for seven SheerView accessories (cordless telescoping pole; rechargeable-with-wand single/multi-channel remotes, 6/10-foot USB chargers, replacement wand, WiFi hub) and eleven Portfolio accessories (LI remote/wall switch/hub/charger; Somfy Situo 1/5, Telis 16, 1/5-channel wall switches, TaHoma, charger). Shared validation rejects fractional/negative quantities and incompatible controls. Control/style changes clear old accessory quantities.

Source retail schedules retain immutable source IDs and PDF page references: SheerView pages 24–27; Portfolio pages 25–26. The staff disclosure separately lists control, valance, oversize and accessory evidence with a subtotal explicitly excluding the base shade, account factors, tax and unresolved order charges. It never grants customer pricing eligibility. SheerView width strictly greater than 93 inches adds the printed $110 retail freight item; exactly 93 does not. Portfolio retains the printed asymmetric $380/$40 Somfy 1/5-channel switch entries rather than guessing a correction; reverify those with the dealer before using a manual quote.

Multi-shade motor/control multiplication remains unresolved and blocked. Blackout's 10% of source shade price is identified separately, not incorrectly added as 10% of accessories. Shared remote/charger/hub quantities must be allocated to one line and reviewed order-wide; no automatic deduplication or complete motor assembly claim is made.

## SheerView painted finishes and mounting envelope

All 11 flat-square painted finishes from source PDF page 10 are selectable by exact code/name (LD-322 Beige through LD-ZY090 Sage). Changing away from Flat Square clears the finish; the shared validator rejects an unknown or stale code. Source PDF page 11 was visually inspected: headrail/bracket depth is 3.7 inches flat, 4.2 curved, and 2.7 No Drill. These are enforced for a requested flush inside recess. The guide does not establish a minimum partially recessed bracket-support depth, so partial recess is saved with an explicit manufacturer-confirmation hold rather than inferred from the housing drawing. Mount and depth/recess choices persist. Page 28's ⅛–3/16-inch factory inside width deduction is disclosed and never automatically subtracted from order measurements. Validation: 426 focused tests plus TypeScript passed.
