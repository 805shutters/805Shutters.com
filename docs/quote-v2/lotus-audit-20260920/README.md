...
Exact finished measurement basis is retained without an inside deduction. Vanes-only hides headrail/stack/wand controls and preserves the unresolved per-vane versus carton basis. Complete steel and one-way aluminum use the documented matching valance; custom center-draw package inclusion is explicitly unresolved rather than inferred. New typed records reject incompatible identity, colors, stack, wand, valance, measurement basis or motors. Older untyped history stays readable. All seven retain restriction-source-incomplete pricing holds; no rate, shipping charge or selling policy changes. These controls are deployed and tested. Representative steel complete60×72White Outside Left48-inchWhite-wand selections saved and reopened on1acd1e85; color/stack/wand survived a full reload. All prices remain held. All seven vertical programs now have individual production save/reopen proof through 418226b6: three complete blinds, three width-only headrails and length-only vanes. The component forms require only their natural measurement axis; exact programs/options persist, valid sibling prices remain, and customer output stays blocked for held lines. See production-proof.md and the configuration ledger.
