...
This resolves the blanket AMX restriction status only for new typed configurations; old snapshots and prices remain unchanged. Outside/side mounting, unsupported motors/valances, impossible donor cuts and unavailable cells remain blocked. The September 20 CAMX2772W cart comparison remains representative current-price evidence. It does not establish the controlling effective date for the entire price book, final freight or applicability of checkout/order-level dealer charges. Existing 805 selling-price policy is preserved. The typed route deployed in f0203d90; price verification awaits the mixed-draft dimension isolation repair discovered in live proof.
...
`amx-boundary-color-ledger.csv` additionally enumerates both colors at all136AMX grid boundaries (272 cases).169have both an exact color ordering code and a documented stock donor at that boundary;50lack the color ordering code;29have a custom ordering code but no matching donor at that exact boundary;24are the two-color views of12unavailable cells. These boundary exceptions stay blocked under the typed rules. Interior dimensions can have different donor feasibility and are checked by the runtime; no live inventory is inferred. Reproduce with `scripts/audit-lotus-amx-boundaries-20260920.py`.
