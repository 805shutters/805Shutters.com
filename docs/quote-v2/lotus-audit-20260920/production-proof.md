...
## Exact parts intended-model increment — deployment/live proof pending

Reviewed all162 observed parts rows. Thirty-four current rows explicitly name compatible MLX/RLX/AMX/RS/RTX/FLX/FGX models in their exact dealer descriptions. A separate additive source-ID map records those descriptions and URLs without modifying the pinned3,710-row registry or inventing missing SKUs. A new part selection captures the intended model from only that item's named choices, plus an optional installed mechanism reference. Server validation rejects a missing/wrong model, a cross-product record or an unsupported typed part. Existing untyped/historical records remain readable.

The remaining128 observations (including the two explicit discontinued rows) do not receive guessed model compatibility from SKU prefixes or generic faux-wood wording. In particular, the old-spec tilter is not silently equated with a current tilter; the `USE MLXHOLDDOWNCL` replacement instruction alone does not establish its own compatible model. All parts retain manual-price/availability/installed-revision-and-fit confirmation holds. The typed model narrows the intended family; it is not proof that a part fits a particular installed mechanism.

Eighteen focused tests pass, including every mapped row/model, wrong/missing model, source-description equality, unsupported/old/discontinued cases, UI filtering and native price-save RPC evidence. The native save retains intended model and mechanism reference with protected validation evidence; the authoritative price snapshot remains null. Local changed-file typecheck is clean with existing unrelated DB/PDF dependency gaps. Real CRM save/reopen proof awaits deployment.

## Source-directed two independent faux blinds — implementation, live proof pending

West A26.v1 pp99/100/102/103 explicitly mark wide/tall FLX/FLXE/FCX/FPX grid cells “Use Two Blinds.” The original UI offered only one or three. The opt-in `lotus-two-independent-blinds-v1` route records both actual component widths, prices each against its own cell at the recorded height and retains physical quantity two. It does not infer equal widths, common-headrail geometry or permissible donor cutting. FTX/FTXLG/FGX are not added to this source-specific route. Earlier one/three-blind selections retain their behavior.

An end-to-end test exposed that FPX had no customer-delivery hold despite unresolved current availability in the full observed assortment. FPX now retains guide-price evidence but requires dealer confirmation of current availability and exact orderable SKUs; this is not a discontinued classification. FLX/FLXE/FCX existing source-price-conflict holds remain.
