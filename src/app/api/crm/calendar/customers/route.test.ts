...
function request(query: string) {
  return new NextRequest(`http://localhost/api/crm/calendar/customers?${query}`);
}

describe("calendar existing customer search", () => {
  ...
  it("escapes PostgREST wildcard and syntax input and returns an explicit cursor", async () => {
    const rows = Array.from({ length: 31 }, (_, index) => ({
      ...
      address: "",
      city: "Ventura",
      product_interest: "Roller Shades", sales_owner: "Mike", lead_source: "Referral", notes: "Gate 1234", status: "completed",
    }));
    ...
    expect(body.results[0]).toMatchObject({jobId: "job-0", city: "Ventura", productInterest: "Roller Shades", assignedTo: "Mike", leadSource: "Referral", notes: "Gate 1234", status: "completed"});
  });
