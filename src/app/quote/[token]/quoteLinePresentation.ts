...
const EMPTY_VALUES = new Set(["", "none", "no", "n/a", "na", "not applicable", "not selected"]);
...
/**
 * Convert saved quote metadata into one concise customer-facing row per category.
 * The source data remains intact; this only removes implementation identifiers and
 * repeated labels from the rendered contract.
 */
export function quoteProductDetails(styleName: string, options: string[]): QuoteProductDetail[] {
  const parsed = options.map(splitDetail).filter(({ label, value }) => label && value && !EMPTY_VALUES.has(normalized(value)));
  ...
  for (const detail of parsed) {
    ...
    if (INTERNAL_DETAIL_LABELS.has(labelKey) || labelKey.endsWith(" id")) continue;
