...
export function SignQuote({ token, customerName }: { token: string; customerName: string }) {
  ...
  async function submit() {
    ...
    try {
      const res = await fetch(`/api/quote/${token}/accept`, {
        ...
        body: JSON.stringify({ printedName: name.trim(), signature: name.trim() }),
      });
  ...
  if (done) {
    return (
      <div style={{ background: "#ecfdf5", border: "1px solid #6ee7b7", borderRadius: 10, padding: 20, marginTop: 20 }}>
  ...
  return (
    <div style={{ border: "1px solid #e2e8f0", borderRadius: 10, padding: 20, marginTop: 20 }}>
      ...
      <label style={{ display: "block", marginBottom: 12 }}>
        ...
        <input
          ...
          style={{ width: "100%", maxWidth: 360, padding: "10px 12px", fontSize: 16, border: "1px solid #cbd5e1", borderRadius: 8, fontFamily: "cursive" }}
        />
      </label>
