...
describe("V2 exact-interface contract", () => {
  it("renders the reusable customer-safe manual quote badge", () => {
    ...
    expect(html).toContain("No Polar price");
    expect(html).toContain('data-testid="manual-quoting-only"');
  });
  ...
  it("persists an auditable Polar QUOTE ONLY task and zero price", () => {
    ...
    expect(buildCatalogSelectionPatch({}, polar)).toMatchObject({
      ...
      options_json: {
        ...
        quote_only_blocks: expect.arrayContaining([
          "pricing",
          "customer_send",
          "order_preparation",
          "manufacturer_action",
  ...
  it("normalizes the CRM-authenticated catalog for manufacturer and motor routing", () => {
    ...
    expect(source).toContain('data-testid="polar-interior-options"');
    expect(source).toContain('data-testid="polar-interior-fabric"');
    expect(source).toContain('data-testid="polar-interior-motorization"');
    expect(source).toContain('"polar-drapery-options"');
    expect(source).toContain('data-testid="polar-drapery-bracket"');
    expect(source).toContain('"polar-drapery-motorization"');
    expect(source).toContain('"polar-awning-options"');
    expect(source).toContain('data-testid="polar-awning-frame-color"');
    expect(source).toContain('data-testid="polar-awning-operation"');
    expect(source).toContain('data-testid="polar-awning-motor"');
    expect(source).toContain('"polar-awning-controls"');
    expect(source).toContain(
      "Only exact suggested-retail grids and compatible published adders",
    );
    expect(source).toContain(
      "The dealer book lists fabric styles, not a color inventory.",
    );
  ...
  it("does not render cross-manufacturer option panels before an exact route exists", () => {
    ...
    for (const [productId, productType] of [
  ...
  it("shows the compact chooser only for V2 categories with multiple usable products", () => {
    ...
    const quoteOnly = catalogProduct("polar_tension", "Polar", [], {
      priceBasis: "manual_required",
    });
    const products = [norman, polar, lotus, unavailable, empty, quoteOnly];
...
describe("Norman shutter source-backed program choices", () => {
  ...
  it("offers the selected program's finishes and excludes AquaShield's unavailable louver", () => {
    ...
    const colors = (program: string) => values(program, "color");
