...
export function JobFinancialStrip({item}:{item:OperationsItem}) {
  const source=item.source; const costs=productOrderCosts(orderCostParent(item)?.meta); const assigned=[...new Set((item.products.length ? item.products : [item.wholeJob]).map(product => orderCostKey(product.records)))].reduce((sum,key)=>sum+(costs[key]?.amount || 0),0);
