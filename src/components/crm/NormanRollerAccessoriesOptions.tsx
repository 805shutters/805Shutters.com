...
export function NormanRollerAccessoriesOptions({design,onUpdateFields}:{design:SalesQuoteDesign|undefined;onUpdateFields:(fields:Partial<SalesQuoteDesign>)=>void}){
 ...
 {([['leftClearance','Roller left magnet clearance',.5625],['rightClearance','Roller right magnet clearance',.5625],['bottomClearance','Roller bottom magnet clearance',.6875]] as const).map(([field,label,min])=><label key={field} className="block text-sm">{label} (minimum {min} inches)<input className={cls} aria-label={label} type="number" step="any" value={r[field]??''} onChange={e=>edit({[field]:e.target.value===''?null:Number(e.target.value)})}/></label>)}
 <p className="text-xs">Measure beyond each finished shade side and below the shade bottom or sill at both catches. Magnets attach to the rear shade on Dual; catches install on site. Cassette includes magnetic hold-downs.</p></>}
 <button className={cls} type="button" disabled={!rollerAccessoryDirty(state)} onClick={()=>{setState(s=>({...s,submitted:s.record}));onUpdateFields({options_json:{...options,[KEY]:state.record}});}}>Save Roller accessories</button><p role="status">{rollerAccessoryDirty(state)?'Unsaved Roller accessories':state.submitted?'Roller accessories submitted':'No unsaved Roller accessories'}</p>
 </section>;
}
