...
import { ROMAN_YARDAGE, ROMAN_ANCILLARY_RECORD, ROMAN_ANCILLARY_HOLD, parseRomanAncillary, romanAncillaryFabrics, pillowSizes, pillowInsertSizes, romanAncillaryRetailReference, type RomanAncillaryRecord } from '@/lib/quote/norman-roman-ancillary';
...
export function NormanRomanAncillaryOptions({design,productId,onUpdateFields}:{design:SalesQuoteDesign|undefined;productId:string;onUpdateFields:(fields:Partial<SalesQuoteDesign>)=>void}){
  ...
  return <section data-testid="norman-roman-ancillary-options" className="space-y-3 rounded-lg border border-slate-200 p-3">
    ...
    <p role="status" className="text-sm text-amber-900">{ROMAN_ANCILLARY_HOLD}</p>
    ...
    {record.kind==='yardage'?<>
      ...
      <p className="text-xs text-slate-600">Maximum 10 yards; line quantity must be one cut. Fractional-yard requests are saved for dealer confirmation because the guide does not state ordering increments.</p>
    </>:<>
    ...
    <p className="text-sm">September suggested-retail reference per {yardage?'cut':'cover'}: {reference===null?'Unverified or selection incomplete':`$${reference.toFixed(2)}`}. This is not an approved customer price or dealer cost.</p>
  </section>;
}
