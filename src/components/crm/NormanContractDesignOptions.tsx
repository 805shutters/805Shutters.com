...
export function NormanContractDesignOptions({design,productId,lineItem,onUpdateFields}:{design:SalesQuoteDesign|undefined;productId:string;lineItem:SalesQuoteLineItem;onUpdateFields:(fields:Partial<SalesQuoteDesign>)=>void}) {
  ...
  return <section className="space-y-3 rounded-lg border border-slate-200 p-3" data-testid="norman-contract-design-options">
    ...
    {select("Contract wand drop",options.contract_wand_drop_inches==null?"Standard":String(options.contract_wand_drop_inches),["Standard",...(vertical?[34,49,61]:[11.75,17.75,29.75,38.25,47.25]).map(String)],v=>update({contract_wand_drop_inches:v==="Standard"?null:Number(v)}))}
    <p className="text-xs">Standard wand drop: {contractStandardWandDrop(productId,height-(inside&&vertical?.1875:0))} inches from the top of the headrail.</p>
    ...
    {inside&&select("Contract mount fit",options.contract_mount_fit,vertical?CONTRACT_VERTICAL_FITS:CONTRACT_FITS,v=>update({contract_mount_fit:v,contract_return_inches:null}))}
    {inside&&number("Contract recess depth (inches)","mount_depth_inches")}
    {!vertical&&design?.valance&&design.valance!=="None"&&<><p className="text-xs">Optional custom measurements are inside dimensions. Leave blank for Norman's standard dimensions.</p>{number("Custom valance length (inches)","contract_valance_length_inches")}{!(inside&&options.contract_mount_fit==="Fully Inside")&&number("Custom valance return (inches)","contract_return_inches")}</>}
