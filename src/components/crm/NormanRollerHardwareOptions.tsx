...
export function NormanRollerHardwareOptions({design,onUpdateFields,pricingOnly=false}:{pricingOnly?:boolean;design:SalesQuoteDesign|undefined;onUpdateFields:(fields:Partial<SalesQuoteDesign>)=>void}){
 ...
 return <section data-testid="norman-roller-hardware" className="space-y-3 rounded-lg border border-slate-200 p-3">
  ...
  {!pricingOnly&&<p className="text-xs text-slate-600">Use a confirmed physical diameter for shared hardware. The fabric appendix may say All Tubes; that classification does not identify the factory-selected tube. This does not change the fabric grid or confirm factory bracket selection.</p>}</>}
  ...
  <label className="flex gap-2 text-sm"><input type="checkbox" checked={state.record.raceway} onChange={e=>edit({raceway:e.target.checked})}/>Optional raceway (no valance)</label>
  ...
  {rollerHardwareDirty(state)&&<div className="roller-option-actions" data-unsaved="true"><button type="button" className={cls} disabled={!rollerHardwareDirty(state)} onClick={save}>Save Roller hardware</button><span role="status">Unsaved changes</span></div>}
 </section>;
}
