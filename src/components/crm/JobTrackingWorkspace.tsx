...
import { JobAdjustmentFields } from "./JobAdjustmentFields";
import { jobAdjustmentFields } from "@/lib/crm/job-adjustment";
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (kind === "stage") {
        ...
        if (kind === "adjustment") {
          const latest = items.find(candidate => candidate.id === item.id);
          if (!financialSource(item) || !latest || latest.balanceOutstanding !== item.balanceOutstanding) throw new Error("This balance changed. Close this editor and review the latest balance before adding an adjustment.");
          const direction = value("adjustment_kind");
          if (direction !== "credit" && direction !== "charge") throw new Error("Choose credit or added charge.");
          patch = financialPatch(item, jobAdjustmentFields(item.balanceOutstanding, value("amount"), direction, value("note")));
  ...
  return <section style={props.editorOnly ? { display: "none" } : undefined} data-focused={Boolean(props.focusedItemId)} className={styles.workspace} aria-labelledby="job-tracking-heading" aria-busy={props.busy}>
          ...
          {visible.map((item) => {
            ...
            return <Fragment key={item.id}>
              <tr>
                ...
                <td><div className={styles.actions}><button type="button" className={styles.primarySmall} disabled={disabled || !canEditMoney} title={paymentReason} onClick={() => open(item, "payment", item.depositOutstanding && item.depositOutstanding > 0 ? "deposit" : "balance")}>Record payment</button><button type="button" className={styles.secondarySmall} disabled={disabled || Boolean(squareReason) || !item.depositOutstanding || item.depositOutstanding <= 0} title={squareReason || "Review and send the deposit link"} onClick={() => open(item, "square", "deposit")}>Send deposit link · {money(item.depositOutstanding)}</button><button type="button" className={styles.secondarySmall} disabled={disabled || Boolean(squareReason) || !item.squareBalanceOutstanding || item.squareBalanceOutstanding <= 0} title={squareReason || "Review and send the outstanding balance link"} onClick={() => open(item, "square", "balance")}>Send balance link · {money(item.squareBalanceOutstanding)}</button>{(squareReason || paymentReason) && <small>{squareReason || paymentReason}</small>}</div></td>
              </tr>
    ...
    <div className={styles.cardList} role="region" aria-label="Job tracking cards">
      {visible.map((item) => {
        ...
        if (props.focusedItemId) return <JobRecordDetail key={item.id} item={item} disabled={disabled} onEdit={(kind, paymentType) => open(item, kind, paymentType)} onFulfillment={() => setFulfillmentItem(item)} />;
        return <article className={styles.jobCard} key={item.id} aria-label={`Job for ${item.customerName}`}>
          ...
          <div className={styles.cardActions}>
            ...
            <button type="button" className={styles.secondarySmall} disabled={disabled || Boolean(squareReason) || !item.depositOutstanding || item.depositOutstanding <= 0} title={squareReason || "Review and send the deposit link"} onClick={() => open(item, "square", "deposit")}>Send deposit link · {money(item.depositOutstanding)}</button>
            <button type="button" className={styles.secondarySmall} disabled={disabled || Boolean(squareReason) || !item.squareBalanceOutstanding || item.squareBalanceOutstanding <= 0} title={squareReason || "Review and send the outstanding balance link"} onClick={() => open(item, "square", "balance")}>Send balance link · {money(item.squareBalanceOutstanding)}</button>
          </div>
    ...
    <Dialog.Root open={Boolean(editor)} onOpenChange={(isOpen) => { if (!isOpen) close(); }}>
      <Dialog.Portal><Dialog.Overlay className={styles.overlay} /><Dialog.Content className={`${styles.modal} ${props.focusedItemId ? styles.darkModal : ""}`} onCloseAutoFocus={(event) => { event.preventDefault(); opener.current?.focus({ preventScroll: true }); }} onEscapeKeyDown={(event) => { if (running) event.preventDefault(); }} onPointerDownOutside={(event) => event.preventDefault()}>
