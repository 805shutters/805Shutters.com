...
export function NormanShutterPanelOptions({design,onUpdateFields}:{design:SalesQuoteDesign|undefined;onUpdateFields:(fields:Partial<SalesQuoteDesign>)=>void}){
 ...
 return <section data-testid="norman-shutter-panel-options" className="space-y-3 rounded-lg border border-slate-200 p-3">
  ...
  {record.application==='specialty'&&<NormanShutterSpecialtyOptions value={record.specialty} programId={program?.id??''} panelCount={record.panels.length} onChange={specialty=>update({specialty})} onPanelCount={n=>update({panels:Array.from({length:n},(_,i)=>record.panels[i]??{heightInches:null,widthInches:null,divider:''})})}/>}
