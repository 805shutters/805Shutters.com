"use client";
import { emptyNormanBifold180Construction, normanBifold180Geometry, normanBifold180BaseboardAdvice, normanBifold180FramingReference, type NormanBifold180Construction } from '@/lib/quote/norman-shutter-bifold180-construction';
export function NormanBifold180ConstructionOptions({value,aqua,onChange}:{value:NormanBifold180Construction|undefined;aqua:boolean;onChange:(value:NormanBifold180Construction)=>void}){
 ...
 const geometry=normanBifold180Geometry(r),advice=normanBifold180BaseboardAdvice(r),framing=normanBifold180FramingReference(r.headerInches);
 ...
 return <section className="space-y-3 rounded border border-slate-200 p-3" aria-label="Bi-fold 180 header and casing construction">
  ...
  {framing&&<p className="text-sm">One standard track is supplied preinstalled on the header. Framing section sizes: top fascia {framing.topFasciaSectionInches} inches; L-shape light-block strip {framing.lightBlockSectionInches} inches. These are section sizes, not cut lengths. The manufacturer must confirm hardware quantities and the pricing measurement basis.</p>}
