...
export function SundanceSharedAccessories({productId,options,onChange}:{productId:string;options:Record<string,unknown>;onChange:(options:Record<string,unknown>)=>void}){
 ...
 <label>Connected motor<select className={classes} aria-label={`Sundance shared accessory ${index+1} motor target`} value={a.targetId} onChange={e=>patch(index,{targetId:e.target.value,accessoryKey:''})}><option value="">Select motor</option>{targets.map(t=><option key={t.id} value={t.id}>{t.label}</option>)}</select></label>
 <label>Accessory<select className={classes} aria-label={`Sundance shared accessory ${index+1} type`} value={a.accessoryKey} onChange={e=>patch(index,{accessoryKey:e.target.value})}><option value="">Select accessory</option>{rows.map(row=><option key={row.key} value={row.key}>{row.label} · ${row.unitSource} source {row.basis}{row.review?' · confirmation required':''}</option>)}</select></label>
