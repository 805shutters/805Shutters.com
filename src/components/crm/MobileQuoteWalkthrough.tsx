...
import { mobileMeasurementPatch, MobileMeasurementKeypad } from "./MobileMeasurementKeypad";
...
export function MobileQuoteWalkthrough({ session, onSessionExpired }: { session: Session; onSessionExpired: () => void }) {
  ...
  /><MobileMeasurementKeypad
    ...
    heightFraction={active.heightFraction}
    onWholeChange={(side, whole) => updateWindow(mobileMeasurementPatch(side, whole))}
    onFractionChange={(side, fraction) => updateWindow({ [side === "width" ? "widthFraction" : "heightFraction"]: fraction })}
