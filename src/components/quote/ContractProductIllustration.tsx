...
export function ContractProductIllustration({ productType, options = [] }: { productType: string; options?: readonly string[] }) {
  ...
  return (
    <figure className={styles.figure} data-contract-illustration="c-v1">
      {panels ? <div className={styles.shutterPanels} style={{width: `min(100%, calc(${panels} * var(--shutter-panel-width)))`}} role="img" aria-label={art.alt} data-panel-count={panels}>
        {Array.from({ length: panels }, (_, index) => <img key={index} src={art.src} alt="" className={styles.shutterPanel} style={{ width: `${100 / panels}%` }} />)}
      </div> : <img src={art.src} alt={art.alt} width={160} height={160} className={styles.product} style={art.mirror ? { transform: "scaleX(-1)" } : undefined} />}
