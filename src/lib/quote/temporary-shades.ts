...
export function temporaryShadeSelected(options: readonly string[] = []): boolean {
  const values = options.flatMap(option => {
    ...
    if (colon < 0 || !["temporary shade", "complimentary temporary shade", "complementary temporary paper shade"].includes(option.slice(0, colon).trim().toLowerCase().replace(/[_-]/g, " "))) return [];
