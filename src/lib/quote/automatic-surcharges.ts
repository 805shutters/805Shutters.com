...
export function deriveAutomaticSurcharges(productId: string, details: DetailRecord | null | undefined): AutomaticSurchargeSelection[] {
  ...
  if (["smartfold", "synchrony_vertical", "perfectsheer", "vertical_honeycomb"].includes(productId) && Number(source.shim_quantity) > 0) add("shim", Number(source.shim_quantity));

  if (["smartfold","perfectsheer","smartdrape"].includes(productId) && Number(source.keystone_quantity) > 0) add("keystone", Number(source.keystone_quantity));
