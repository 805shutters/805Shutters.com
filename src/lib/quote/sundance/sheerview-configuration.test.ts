...
const options={...sundanceSheerviewColorPatch({},'sundance_sheerview:S50PN100')!,sundance_sheerview_control:'Continuous Cord Loop',sundance_sheerview_headrail:'Curved',sundance_sheerview_cord_option:'Cord',sundance_sheerview_assembly:'Single',mount_type:'Outside'};
...
it.each([
 ...
 const changes={sundance_sheerview_control:control,sundance_sheerview_headrail:headrail,sundance_sheerview_headrail_finish:headrail==='Flat Square'?'LD-S113':null,sundance_sheerview_cord_option:control==='Continuous Cord Loop'?'Cord':null,catalog_sundance_sheerview_valance_id:headrail==='Flat Square'?'sundance_sheerview_valance_p24_t3':null};
...
it('enforces RD flat headrail84-inch limit and rejects forged material routes',()=>{
 ...
 const c=context({...rd,sundance_sheerview_headrail:'Flat Square',sundance_sheerview_headrail_finish:'LD-S113',catalog_sundance_sheerview_valance_id:'sundance_sheerview_valance_p24_t3'},36,84);c.programId=String(rd.catalog_program_id);
...
it('checks exact painted finishes and flush mounting envelope without inventing a partial-depth minimum',()=>{
 const flat={sundance_sheerview_headrail:'Flat Square',sundance_sheerview_headrail_finish:'LD-S113',catalog_sundance_sheerview_valance_id:'sundance_sheerview_valance_p24_t3',mount_type:'Inside',sundance_sheerview_mount_depth:3.7,sundance_sheerview_recess:'Flush'};
 expect(validateSundanceSheerviewConfiguration(context(flat))).toEqual([]);
 expect(validateSundanceSheerviewConfiguration(context({...flat,sundance_sheerview_mount_depth:3.69})).map(i=>i.ruleId)).toContain('sundance.sheerview.flush_depth');
 expect(validateSundanceSheerviewConfiguration(context({...flat,sundance_sheerview_recess:'Partial recess'})).map(i=>i.ruleId)).toContain('sundance.sheerview.recess_review');
 expect(validateSundanceSheerviewConfiguration(context({...flat,sundance_sheerview_headrail_finish:'UNKNOWN'})).map(i=>i.ruleId)).toContain('sundance.sheerview.flat_finish');
 expect(sundanceSheerviewHeadrailPatch(flat,'Curved')).toMatchObject({sundance_sheerview_headrail_finish:null});
});
