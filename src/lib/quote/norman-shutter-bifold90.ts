import {normanSpecialBifoldLayouts,parseNormanFloating90,parseNormanFrameHinged,type NormanFloating90,type NormanFrameHinged} from './norman-shutter-bifold-special';
export type NormanBifold90Record={version:1;kind:''|'standard_90'|'multifold_90'|'floating_90'|'frame_hinged';layout:string;mount:''|'Inside Mount'|'Semi-Inside Mount'|'Outside Mount';casing:''|'none'|'existing';referenceWidthInches:number|null;referenceHeightInches:number|null;headerInches:3|3.5|null;fascia:''|'plain'|'deco';headerExtensionInches:number|null;flatMountingSurface:boolean;floating?:NormanFloating90;frameHinged?:NormanFrameHinged};
...
export function normanBifold90Layouts(p:string,kind:string):readonly string[]{return kind==='standard_90'?['LL','RR','LLRR']:kind==='multifold_90'&&normanBifold90Wood(p)?['LLLL','RRRR','LLLLRRRR']:normanSpecialBifoldLayouts(p,kind);}
...
export function parseNormanBifold90(value:unknown):NormanBifold90Record|null{
 ...
 if(r.floating!==undefined&&!parseNormanFloating90(r.floating)||r.frameHinged!==undefined&&!parseNormanFrameHinged(r.frameHinged))return null;
