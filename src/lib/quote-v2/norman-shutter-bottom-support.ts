...
/** WL33 / WLP38 / BW32 / ND32: "Shutters to be installed where there is no bottom supports are not available."
 * The following list identifies gap over0.1 and no frame/sill as unsupported.
 * Its frequently-left-open parenthetical describes sag/deformation risk: retain that
 * operational caution as a warning, not an unconditional order-availability ban.
 */
export function validateNormanShutterBottomSupport(s:SelectionContext,record:NormanShutterPanelRecord):ValidationIssue[]{
 ...
 record.panels.forEach((panel,index)=>{
  ...
  if(r.frequentlyOpen)add('frequent_open',index,'the guide warns that panels frequently left open may sag or deform even with a bottom frame/sill; review intended use and support with the manufacturer.','warning');
 });
 return issues;
}
