...
export const currentSmartfoldEligibility=(s:SelectionContext)=>s.productId==="smartfold"&&s.catalogAsOf>="2026-09-20"&&s.catalogVersion.endsWith("-norman-smartfold-outside-2026-09-20-r8");
export function smartfoldBranchExceptions(s:SelectionContext):string[]{
 ...
 if(norm(c.lift_system)!=="motorized"||norm(c.motor_type)!=="norman smart rechargeable battery ac charger")reasons.push("this verified branch uses Norman Smart rechargeable battery with AC charger");
