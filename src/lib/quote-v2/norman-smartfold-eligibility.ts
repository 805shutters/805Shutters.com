import {smartfoldInsideLightGuardPricingBranch,smartfoldInsideLightGuardClearance,validateSmartfoldInsideLightGuardClearance} from "./norman-smartfold-inside-clearance";
...
function branchExceptions(s:SelectionContext,quoteOnly=false):string[]{
 ...
 // Matching groups and door placement change fabrication readiness, not individual grid/option charges.
 ...
 if(!["","single","single shade","standard"].includes(norm(c.shade_type))||/specialty|day night/.test(norm(c.application))||(!quoteOnly&&(active(c.installed_on_door)||active(c.door_application)||/door/.test(norm(c.application)))))reasons.push("only individual standard shades are verified; door applications were removed in July");
 if(active(c.smartfold_common_valance_id)||(!quoteOnly&&(active(c.smartfold_side_by_side_id)||active(c.side_by_side)||c.side_by_side_match_line_id)))reasons.push("common-valance/side-by-side assemblies need separate branch verification");
 if((active(c.basic_light_guard)||active(c.light_guard))&&!(quoteOnly?smartfoldInsideLightGuardClearance(s)!==null&&["none","no valance"].includes(norm(c.valance)):smartfoldInsideLightGuardPricingBranch(s)))reasons.push("Light Guard requires its inside-mount verification");
 ...
 * Callers must still enforce grid, dimensional, quantity and priced-option validation.
 */
...
export function validateSmartfoldEligibility(s:SelectionContext):ValidationIssue[]{
 ...
 if(smartfoldInsideLightGuardPricingBranch(s))return validateSmartfoldInsideLightGuardClearance(s);
