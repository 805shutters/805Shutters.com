...
export function validateNormanShutterDividers(s:SelectionContext,record:NormanShutterPanelRecord):ValidationIssue[]{
 ...
 const pages=program.id.startsWith('woodlore_')?[44]:program.id.startsWith('normandy')?[38,39]:[38];
 ...
 record.panels.forEach((panel,index)=>{
  if(panel.divider!=='present')return;
  ...
  if(!r.measurementBasis||r.referenceHeightInches===null||r.referenceHeightInches<=0)add('measurement_basis',index,'record the window or max-frame datum and its actual reference height.');
  if(r.rails.length===0)add('rail_required',index,'divider presence requires at least one rail in the saved schedule.');
  ...
  if(r.splitTiltCentersInches.some(p=>p<=0||r.referenceHeightInches===null||p>=r.referenceHeightInches))add('split_position',index,'split-tilt centers must lie within the same measured window/frame reference height.');
