...
export function ultimateFauxComponents(s:SelectionContext) {
 ...
 const wand=Number(c.ultimate_wand_drop??ultimateFauxWandDrop(h));
 ...
 if(custom!==null&&(!hasValance||widths.length!==1||!Number.isFinite(custom)||custom<=0||custom>(netWidths[0]??0)+5))add("custom_valance",11,"Custom valance width must be positive and no greater than net blind width plus 5 inches. Use separate lines for independent custom valances.");
 ...
 if([c.common_valance,c.side_by_side].some(yes)||/common|2.on|3.on|two.on|three.on/i.test(String(c.application??c.shade_type??"")))add("assembly",12,"Common valance and side-by-side matching require an order assembly with each blind, gap and matching specification; assembly setup is not yet available on this line.");
 ...
 const valanceWidth=custom??(netWidths[0]??0)+(inside?returns!=="None"?.625:.25:returns!=="None"?1:2);
 ...
 valance:hasValance?{style:valance,width:custom??netWidth+(inside?returns!=="None"?.625:.25:returns!=="None"?1:2),returns,returnSize,returnConnectors:(returns==="Both"?2:returns==="None"?0:1)*(valance==="3.25-inch Designer Crown"?2:1)}:null}));
 ...
 return {issues,surchargeSelections,record:{version:1,type:"ultimate_faux_independent_blinds",sourceId:ULTIMATE_FAUX_SOURCE,sourcePages:[6,7,8,9,10,11,14,15],components,cutouts,keystones:{count:keystoneCount,layout,locations},color:color?{...color,factoryCode:ULTIMATE_FAUX_FACTORY_CODES[color.code]}:null,wandDrop:wand,mountFit:inside?fit:null,requiredMountDepth:inside?depth:null,shimExtension:layers===2?.6875:layers===1?.375:0}};
}
