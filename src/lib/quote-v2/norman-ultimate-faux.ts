...
export function ultimateFauxComponents(s:SelectionContext) {
 ...
 valance:hasValance?{style:valance,width:common?.finishedWidth??custom??netWidth+(inside?returns!=="None"?.625:.25:returns!=="None"?1:2),returns,returnSize,returnConnectors:(returns==="Both"?2:returns==="None"?0:1)*(valance==="3.25-inch Designer Crown"?2:1)}:null}));
 ...
 return {issues,surchargeSelections,record:{version:1,type:"ultimate_faux_independent_blinds",sourceId:ULTIMATE_FAUX_SOURCE,sourcePages:[6,7,8,9,10,11,14,15],components:components.map(component=>({...component,orderedWidth:finiteOrNull(component.orderedWidth),netWidth:finiteOrNull(component.netWidth),height:finiteOrNull(component.height),valance:component.valance?{...component.valance,width:finiteOrNull(Number(component.valance.width)),returnSize:component.valance.returnSize===null?null:finiteOrNull(component.valance.returnSize)}:null})),commonValance:common,cutouts:cutouts.map(cut=>({...cut,width:finiteOrNull(cut.width),top:finiteOrNull(cut.top),...(cut.bottom!==undefined?{bottom:finiteOrNull(cut.bottom)}:{})})),keystones:{count:finiteOrNull(keystoneCount),layout,locations:locations.map(finiteOrNull)},color:color?{...color,factoryCode:ULTIMATE_FAUX_FACTORY_CODES[color.code]}:null,wandDrop:finiteOrNull(wand),mountFit:inside?fit:null,requiredMountDepth:inside?depth:null,shimExtension:layers===2?.6875:layers===1?.375:0}};
}
