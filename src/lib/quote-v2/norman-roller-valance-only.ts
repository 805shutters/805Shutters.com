...
export function deriveRollerSeparateValances(lines:readonly {lineId:string;selection:SelectionContext}[]):ValidationIssue[]{
 ...
 for(const row of lines.filter(l=>l.selection.productId===SEPARATE)){
  ...
  add('hardware_matching','Large mounting brackets and compatible large tubes must be established for every associated shade. Exact largest-hardware derivation remains pending.');
  s.configuration={...s.configuration,[DERIVED]:{version:1,type:'roller_separate_valance',sourceId:SOURCE,sourcePages:[37],ownerLineId:row.lineId,associatedLineIds:r.associatedLineIds,associatedOrderWidths:members.map(l=>l.selection.widthInches),associatedApplications:apps,associatedLiftSystems:members.map(l=>String(l.selection.configuration.lift_system??'')),recommendedMinimumWidth:minimum,recommendedMaximumWidth:maximum,recommendedWidthStatus:r.width!==null&&r.width>=minimum&&r.width<=maximum?'within_recommended_range':'outside_recommended_range',mount:r.mount,valanceBracketSize:'large',associatedMountingBracketSize:'large',tubeUpgradeRequired:large,pricingStatus:'standalone_separate_price_unverified'}};
 }
