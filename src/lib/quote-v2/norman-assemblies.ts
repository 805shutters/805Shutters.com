...
export function deriveNormanOrderRecords(lines: readonly SmartfoldOrderLine[]): ValidationIssue[] {
  ...
  for (const { selection } of lines) {
    ...
    const smartfoldInsideClearance = smartfoldInsideLightGuardClearance(selection);
    if (smartfold) selection.configuration = { ...selection.configuration, [NORMAN_ASSEMBLY_KEY]: {...smartfold.record,...(smartfoldClearance?{outsideClearance:smartfoldClearance}:{}),...(smartfoldInsideClearance?{insideLightGuardClearance:smartfoldInsideClearance}:{})} };
  ...
  for(const members of motorNetworks.values()) {
    ...
    for(const {line} of members) {
      ...
      const assembly=line.selection.configuration[NORMAN_ASSEMBLY_KEY] as SelectionRecord;
      line.selection.configuration={...line.selection.configuration,[NORMAN_ASSEMBLY_KEY]:{...assembly,motorNetwork:{version:1,network:first.network,family:first.family,connectedLineIds:members.map(m=>m.line.lineId).sort(),controllerQuantity:suppliedRemotes,existingRemoteEvidence:members.some(m=>String(m.accessories.record.controller?.existingRemoteWorkOrder ?? "").trim()),remoteRequired:romanControls.some(m=>m.line.lineId===line.lineId)||perfectsheerControls.some(m=>m.line.lineId===line.lineId),repeaters,capacity,sourceId:"norman-motorization-guide-2026-09-16",sourcePage:networkSourcePage}}};
    }
  }
  for(const {selection} of lines) {
