...
describe('Roman configurator style pricing',()=>{
 ...
 it('does not mutate prior snapshot pricing and removes stale style charges from current plain styles',()=>{
  ...
  expect(authoritativeAutomaticSurchargeSelections({...s,catalogVersion:s.catalogVersion.replace(/-r\d+$/, '-r3')})).not.toContainEqual({id:'soft_fold_edge_banding_border',units:1});
