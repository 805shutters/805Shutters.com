...
import { smartdrapeTrack, validateSmartdrapeTrack } from "./norman-smartdrape-tracks";
import { smartdrapeHardware, validateSmartdrapeHardware } from "./norman-smartdrape-hardware";
...
import { validateSmartfoldValance } from "./norman-smartfold-valance";
...
export function validateNormanFamilyRules(context: SelectionContext): ValidationIssue[] {
  ...
  if (context.productId === "smartdrape" && context.catalogAsOf >= "2026-09-01") {
    ...
    const finishedHeight = context.catalogAsOf >= "2026-09-19" ? smartdrapeHardware(context)?.record.shadeHeight ?? null : h;
    if (w < minWidth || w > maxWidth || h < 24 || h > 144 || (finishedHeight !== null && finishedHeight < 24)) add("dimensions", source, 6, `SmartDrape track width must be ${minWidth}–${maxWidth} inches and shade height 24–144 inches for the selected operation and stack.`);
    if (finishedHeight !== null && (smartdrapeTrack(context)?.shadeWidth ?? (w - (centerOpening ? 5.5625 : sideBySide ? 3.5 : 5.5))) * finishedHeight > 237 * 144) add("area", source, 6, "SmartDrape fabric area cannot exceed 237 square feet.");
