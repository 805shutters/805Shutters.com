...
import { validateSmartdrapeHardware } from "./norman-smartdrape-hardware";
...
export function validateNormanFamilyRules(context: SelectionContext): ValidationIssue[] {
  const issues: ValidationIssue[] = [...(smartprivacyComponents(context)?.issues ?? []), ...(smartdrapeMotorAccessories(context)?.issues ?? []), ...validateSmartdrapeComponents(context), ...validateSmartdrapeHardware(context), ...validateSmartdrapeTrack(context), ...validateSmartdrapeVanePacks(context), ...validatePerfectsheerComponents(context), ...validateSanClemente(context), ...validateNormanContract(context), ...validateSmartfoldHardware(context), ...validateSmartfoldAccessories(context), ...validateSmartfoldStyle(context), ...validateSmartfoldValance(context)];
  ...
  if (context.productId === "smartdrape" && context.catalogAsOf >= "2026-09-01") {
    ...
    if ((smartdrapeTrack(context)?.shadeWidth ?? (w - (centerOpening ? 5.5625 : sideBySide ? 3.5 : 5.5))) * h > 237 * 144) add("area", source, 6, "SmartDrape fabric area cannot exceed 237 square feet.");
