...
export function deriveRomanMatching(lines:readonly SmartfoldOrderLine[]):ValidationIssue[]{
 const active=lines.filter(l=>l.selection.productId==='roman'&&/norman-roman-mounting-2026-09-20-r[6789]$/.test(l.selection.catalogVersion)&&['yes','true'].includes(norm(l.selection.configuration.side_by_side)));
