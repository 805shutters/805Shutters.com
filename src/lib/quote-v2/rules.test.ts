...
describe("Quote V2 authoritative manufacturer rules", () => {
  ...
  it("accepts the three currently verified Caroline styles while preserving historical quarantine and other exclusions", () => {
    for (const fold_style of ["Flat Fold without Seams", "Flat Fold with Batten Back", "Soft Fold"]) {
      ...
      expect(ruleIds({...current,widthInches:96.375})).not.toContain("roman.dimension.width");
      expect(ruleIds({...current,widthInches:96.4375})).toContain("roman.dimension.width");
    }
