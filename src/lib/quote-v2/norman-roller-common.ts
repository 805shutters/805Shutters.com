...
export function deriveRollerCommonValances(lines:readonly {lineId:string;selection:SelectionContext}[]):ValidationIssue[]{
 ...
 for(const [groupId,members] of groups){
  ...
  // The PDF's 95-inch piece limit is also subject to the chosen material roll width.
  if(fabric||wrapped)add("material_width",39,"Confirm the exact selected valance fabric and its roll-width limit before deriving splice sections; the shade's dimensional grid is not a fabric-roll-width table.");
