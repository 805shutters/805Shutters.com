...
export function deriveRollerCommonValances(lines:readonly {lineId:string;selection:SelectionContext}[]):ValidationIssue[]{
 ...
 for(const [groupId,members] of groups){
  ...
  const exactWidths=/-material-2026-09-20-r6$|-group-hardware-2026-09-20-r7$/.test(s.catalogVersion);
  ...
  const hardware=s.catalogVersion.endsWith("-group-hardware-2026-09-20-r7")?deriveRollerGroupHardware(sorted,"common"):null;
