...
import {validateSundanceSharedAccessories} from "@/lib/quote/sundance/order-accessories";
...
import { validateSundanceVerticalConfiguration } from "@/lib/quote/sundance/vertical-configuration";
...
export function productRuleStatusForSelection(context: SelectionContext): ProductRuleStatus {
  ...
  if (context.productId === "onyx_shutters" && context.programId === "poly_composite") {
    return "documented_limited";
  }
...
export function validateSelection(context: SelectionContext): readonly ValidationIssue[] {
  ...
  if(context.productId.startsWith("sundance_")) issues.push(...validateSundanceAssembly(context), ...validateSundanceSharedAccessories(context));
