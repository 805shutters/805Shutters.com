...
export function validateNormanShutterAssortment(s: SelectionContext): ValidationIssue[] {
  ...
  if (application !== "regular" && !dedicatedTrack && c.stile_join && normanRegularPanelCount(c.panel_config) === 1 && !(Number(c.widest_panel_width_inches) >= 9)) add("single_panel_join", "Single panels below 9 inches require the exact finished panel width and hang-strip placement to verify the stile join.", constructionPages);
