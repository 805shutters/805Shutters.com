...
export function validateNormanShutterAssortment(s: SelectionContext): ValidationIssue[] {
  ...
  if (application !== "regular" && !dedicatedTrack && mixedBifold && Number(c.widest_panel_width_inches) > normanRegularPanelMaxWidth(p.id, c.louver_size, "LL")) add("mixed_panel_width", "This mixed hinged/bifold layout needs individual finished panel widths to verify the narrower bifold limit.", p.id.startsWith("woodlore_") ? [38] : [32]);
