...
export function validateNormanShutterAssortment(s: SelectionContext): ValidationIssue[] {
  ...
  if (application !== "regular" && application !== "double_hung" && !dedicatedTrack && mixedBifold && Number(c.widest_panel_width_inches) > normanRegularPanelMaxWidth(p.id, c.louver_size, "LL")) add("mixed_panel_width", "This mixed hinged/bifold layout needs individual finished panel widths to verify the narrower bifold limit.", p.id.startsWith("woodlore_") ? [38] : [32]);
  ...
  if (application !== "regular" && !(application === "french_door" && panelRecord?.frenchDoor && ["E", "F"].includes(panelRecord.frenchDoor.cutoutType)) && !dedicatedTrack && c.stile_join && normanRegularPanelCount(c.panel_config) === 1 && !(Number(c.widest_panel_width_inches) >= 9)) add("single_panel_join", "Single panels below 9 inches require the exact finished panel width and hang-strip placement to verify the stile join.", constructionPages);
