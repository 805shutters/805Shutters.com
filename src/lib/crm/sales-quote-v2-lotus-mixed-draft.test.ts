...
const designs = lines.map(row=>({id:`design-${row.id}`,line_item_id:row.id,variant:"A",product_type:row.product_type,supplier:"Lotus",unit_price:0,mount_type:"Inside Mount",options_json:{quote_v2_backend:true,...(row.id==="ftx"?{
