...
import { checkCustomerEmailDeliveries, getCustomerEmailMonitor } from "./customer-email-monitor";
...
it("reports a failed heartbeat even when no email is queued", async () => {
 ...
 const db = { from: (table: string) => {
   const result = table === "crm_activity_events" ? [{created_at: new Date(now).toISOString(), action: "customer-email.failed"}] : [];
