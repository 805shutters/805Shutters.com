...
describe("partner payment write rules", () => {
  ...
  it.each([{ mode: "equal-owner partial", amount: 100 }])("preserves $mode payment metadata when allocation storage fails", async ({ mode, amount }) => {
    ...
    class PartnerQuery {
      ...
      range(from: number, to: number) {
        const result = this.execute(false);
    ...
    const result = await createPartnerPaymentBatch(
      supabase,
      { person: "mike", item_ids: ["mike:crm_quote:quote-1"], ...(mode === "legacy" ? {} : { payment_model: "equal_owners_v1", amount }) },
    ...
    expect((mode === "legacy" ? result.dashboard.partnerPaymentLedger : result.dashboard.ownerPayablesLedger!).people.mike).toMatchObject({
      paid: amount,
      owed: mode === "legacy" ? 0 : 300,
      activeJobCount: mode === "legacy" ? 0 : 1
    });
    if (mode !== "legacy") {
      expect(result.receiptEmail).toBeUndefined();
      const beforeWrites = calls.length;
      await expect(createPartnerPaymentBatch(supabase, { person: "mike", payment_model: "equal_owners_v1", amount: 301, item_ids: ["mike:crm_quote:quote-1"] }, { email: "805shutters@gmail.com" })).rejects.toThrow("exceeds");
      await expect(createPartnerPaymentBatch(supabase, { person: "mike", payment_model: "equal_owners_v1", amount: 10, item_ids: ["mike:crm_quote:missing"] }, { email: "805shutters@gmail.com" })).rejects.toThrow("no active unpaid");
      expect(calls.length).toBe(beforeWrites);
    }
  });
...
describe("rescheduleCrmCalendarEvent admin override", () => {
  const event = {
  ...
  it("requires an authenticated staff actor without an opt-in flag", async () => {
    const { supabase, rpcCalls } = calendarCancelRecorder({ event });
    await expect(rescheduleCrmCalendarEvent(supabase, {
      id: event.id, start_at: event.start_at, end_at: event.end_at,
    }, { email: actor.email })).rejects.toThrow(/authenticated staff/);
    expect(rpcCalls).toHaveLength(0);
  });
  ...
  it("uses only the authenticated actor and records the override audit", async () => {
    ...
    const rpc = rpcCalls.find((call) => call.name === "booking_admin_reschedule");
  ...
  it("reschedules every assignee and blocks without availability or route checks", async () => {
    for (const appointment of [event, { ...event, assigned_to: "Mike" }, { ...event, event_type: "block" as const }]) {
      const { supabase, rpcCalls } = calendarCancelRecorder({ event: appointment });
      await rescheduleCrmCalendarEvent(supabase, {
        id: appointment.id, start_at: appointment.start_at, end_at: appointment.end_at,
      }, actor);
  ...
  it("still rejects invalid time ranges", async () => {
    const { supabase, rpcCalls } = calendarCancelRecorder({ event });
    await expect(rescheduleCrmCalendarEvent(supabase, {
      id: event.id, start_at: event.end_at, end_at: event.start_at,
    }, actor)).rejects.toThrow(/end time must be after/);
    expect(rpcCalls).toHaveLength(0);
  });
});
