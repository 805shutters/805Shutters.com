    ...
    if (!file.products.length) {
      for (const job of file.jobs) {
        ...
        pushUnique(file.products, `job-product-${job.id}`, {
          ...
          status: job.status,
          meta: { source: "crm_job", product_type_workflow: objectMeta(job.meta).product_type_workflow, ordered_at: objectMeta(checks.ordered).at, shipped_at: objectMeta(checks.shipped).at, shipping_confirmation: checks.shipping_confirmation }
        });
      }
    }
