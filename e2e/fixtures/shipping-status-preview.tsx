...
function Preview(){
 ...
 return <main style={{maxWidth:1500,margin:'24px auto',padding:16,fontFamily:'Arial,sans-serif'}}><p>LOCAL PREVIEW · SAMPLE DATA</p><JobStatusOverview data={data} busy={false} onOpen={()=>{}} onSaveCost={async()=>true} onAction={async(_,step,product,invoice,shipment)=>{if(step==='shipped'&&shipment)setProducts(old=>old.map(p=>product?.records.some(r=>r.id===p.id)?{...p,meta:{...p.meta,shipped_at:'2026-09-19',shipping_confirmation:shipment}}:p));}} /><button style={{padding:12,marginTop:12,color:'#111',background:'#eee'}} onClick={()=>setProducts(initial)}>Reset sample</button><details><summary>Navigation preview</summary><CrmNavigation activeTab="tracking" onNavigate={()=>{}} onRefresh={()=>{}} onSignOut={()=>{}} busy={false}/></details></main>;
}
