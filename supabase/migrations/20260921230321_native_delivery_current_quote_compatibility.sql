-- Activate the delivery ledger against current retail snapshots without rewriting
-- existing customer preparations. Unknown supplier costs remain unknown.
do $migration$
declare definition text;
begin
 definition := pg_get_functiondef('public.reserve_native_quote_delivery(uuid,uuid,bigint,text,jsonb,jsonb)'::regprocedure);
 definition := regexp_replace(definition, '(from|join)([[:space:]]+)public[.]sales_quote_line_items\M', '\1\2public.sales_quote_active_line_items', 'gi');
 definition := replace(definition, $old$'quantity',l.quantity,'productTotal'$old$, $new$'quantity',l.quantity,'costStatus',case when public.quote_v2_has_unresolved_quote_cost(s.retail_snapshot,s.internal_cost_snapshot) then 'unresolved' else 'known' end,'productTotal'$new$);
 if position('costStatus' in definition)=0 then raise exception 'Native delivery cost snapshot target changed'; end if;
 execute definition;
end $migration$;

-- Same cent-rounding and discountable-base policy as computeQuoteMoney.
-- Inputs come only from the immutable native mirror and its stored adjustments.
create or replace function public.native_quote_money(p_subtotal numeric,p_fixed numeric,p_adjustments jsonb,p_cost numeric)
returns jsonb language plpgsql immutable set search_path=public,pg_temp as $$
declare a jsonb:=coalesce(p_adjustments,'{}'); extras numeric:=0; discountable numeric;
 discount numeric; taxable numeric; tax numeric; total numeric; deposit numeric; balance numeric;
begin
 if jsonb_typeof(a->'fees')='array' then
  select coalesce(sum(greatest(0,public.quote_control_number(fee->'amount'))),0) into extras from jsonb_array_elements(a->'fees') fee;
 end if;
 extras:=round(extras,2);
 discountable:=greatest(0,round(p_subtotal+extras,2)-greatest(0,p_fixed));
 discount:=round(least(discountable,greatest(0,public.quote_control_number(a->'discountFlat'))+discountable*least(100,greatest(0,public.quote_control_number(a->'discountPercent')))/100),2);
 taxable:=round(greatest(0,round(p_subtotal+extras,2)-discount),2);
 tax:=round(taxable*least(100,greatest(0,public.quote_control_number(a->'taxPercent')))/100,2);
 total:=round(taxable+tax,2);
 if jsonb_typeof(a->'totalOverride')='number' and (a->>'totalOverride')::numeric>=0 then total:=round((a->>'totalOverride')::numeric,2); end if;
 deposit:=round(total*case when a->>'depositPercent' is null then 0 else least(100,greatest(0,public.quote_control_number(a->'depositPercent'))) end/100,2);
 balance:=round(greatest(0,total-deposit),2);
 if jsonb_typeof(a->'balanceDueOverride')='number' and (a->>'balanceDueOverride')::numeric>=0 then balance:=round((a->>'balanceDueOverride')::numeric,2); total:=round(deposit+balance,2); end if;
 return jsonb_build_object('subtotal',p_subtotal,'total',total,'discount',discount,'tax',tax,'sourceTotalAdjustment',0,'depositDue',deposit,'balanceDue',balance,'materialsCost',p_cost,'laborCost',0);
end $$;
revoke all on function public.native_quote_money(numeric,numeric,jsonb,numeric) from public,anon,authenticated;
grant execute on function public.native_quote_money(numeric,numeric,jsonb,numeric) to service_role;
...
create or replace function public.accept_native_quote_delivery(
...
declare q public.crm_quotes%rowtype; d public.sales_quote_v2_deliveries%rowtype; l record; f record;
 ...
 v_fixed numeric:=0; v_future_fixed numeric:=0; v_per_window_fixed numeric; v_unknown boolean;
...
begin
 ...
 for l in select * from public.crm_quote_line_items where quote_id=q.id order by sort_order,id for update loop
  ...
  v_unknown:=v_cost->>'costStatus'='unresolved';
  if v_cents is null or v_cents<0 or (not coalesce(v_unknown,false) and (v_cost_cents is null or v_cost_cents<0 or v_product_cents is null or v_product_cents<0)) then raise exception 'Frozen amounts are invalid.' using errcode='55000'; end if;
  v_n:=0; v_selected_product:=case when v_unknown then null else 0 end; v_selected_cents:=0; v_selected_cost:=case when v_unknown then null else 0 end;
  if v_unknown then v_product_cents:=null; v_cost_cents:=null; end if;
  select coalesce((price_breakdown#>>'{customerCharges,perWindowTotal}')::numeric,0) into v_per_window_fixed from public.crm_quote_designs where id=l.selected_design_id;
  ...
  v_fixed:=v_fixed+coalesce(v_per_window_fixed,0)*v_n;
  v_future_fixed:=v_future_fixed+coalesce(v_per_window_fixed,0)*(l.quantity-v_n);
  ...
  if v_n>0 then
   ...
   v_current_costs:=v_current_costs||jsonb_build_object(l.id::text,jsonb_build_object('quantity',v_n,'costStatus',case when v_unknown then 'unresolved' else 'known' end,'productTotal',v_selected_product/100.0,'total',v_selected_cost/100.0));
  end if;
  if v_n<l.quantity then
   ...
   v_future_costs:=v_future_costs||jsonb_build_object(l.sort_order::text,jsonb_build_object('quantity',l.quantity-v_n,'costStatus',case when v_unknown then 'unresolved' else 'known' end,'productTotal',(v_product_cents-v_selected_product)/100.0,'total',(v_cost_cents-v_selected_cost)/100.0));
  end if;
  ...
  if v_n>0 then v_product_total:=v_product_total+v_selected_product; v_internal:=v_internal+v_selected_cost; end if;
  if v_n<l.quantity then v_internal_remaining:=v_internal_remaining+v_cost_cents-v_selected_cost; end if;
 end loop;
 v_cur_money:=public.native_quote_money(v_total/100.0,v_fixed,q.meta->'adjustments',v_internal/100.0);
 v_fut_money:=public.native_quote_money(v_remaining/100.0,v_future_fixed,q.meta->'adjustments',v_internal_remaining/100.0);
 if not (p_selected_line_ids<@v_known) or v_total<=0 or round(p_acknowledged_total,2)<>(v_cur_money->>'total')::numeric then
 ...
 if v_remaining_quantity>0 then
  ...
  insert into public.sales_quote_v2_deliveries(id,quote_id,preparation_id,crm_quote_id,actor_id,quote_revision,request_key,request,customer_payload,internal_line_costs,share_token)
   values(v_fdid,null,d.preparation_id,v_fqid,d.actor_id,1,'future:'||v_fqid,d.request,
    jsonb_build_object('total',(v_fut_money->>'total')::numeric,'parentDeliveryId',d.id,'lineTotals',v_counts),v_entry,v_token);
  ...
  update public.crm_quotes set status='sold',signed_at=p_signed_at,sold_at=p_signed_at,customer_signature=p_signature,customer_printed_name=p_printed_name,
   quote_total=(v_cur_money->>'total')::numeric,materials_cost=v_internal/100.0,discount=(v_cur_money->>'discount')::numeric,tax=(v_cur_money->>'tax')::numeric,deposit_required=(v_cur_money->>'depositDue')::numeric,balance_due=(v_cur_money->>'balanceDue')::numeric where id=q.id;
 end if;
 ...
 insert into public.sales_quote_v2_acceptances(delivery_id,crm_quote_id,selected_line_ids,line_quantities,accepted_total,accepted_at,future_quote_id,future_job_id)
  values(d.id,q.id,p_selected_line_ids,v_plan,(v_cur_money->>'total')::numeric,p_signed_at,v_fqid,v_fjid);
 update public.sales_quotes set status='sold',signed_at=p_signed_at,customer_signature=p_signature,customer_printed_name=p_printed_name,total_amount=(v_cur_money->>'total')::numeric,manufacturer_cost=v_product_total/100.0,
  quote_v2_accepted_selection=jsonb_build_object('lineQuantities',v_plan,'selectedLineIds',to_jsonb(p_selected_line_ids),'acceptedTotal',(v_cur_money->>'total')::numeric,
