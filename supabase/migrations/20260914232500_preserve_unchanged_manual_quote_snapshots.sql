...
create or replace function public.save_quote_v2_catalog_pricing_batch(
  p_quote_id uuid,
...
begin
  ...
  loop
    ...
    -- An unrelated line edit must not replace an explicit staff price or its
    -- frozen cost/configuration evidence. Edited manual selections retain the
    -- normal catalog + manual-reapply flow in the public batch wrapper.
    if v_select_design and v_line.selected_design_id=v_design_id
      and v_design.quote_v2_selection is not distinct from v_selection
      and exists(select 1 from public.sales_quote_line_price_overrides o
        join public.sales_quote_v2_price_snapshots s on s.id=v_design.current_v2_snapshot_id
        where o.design_id=v_design_id and o.quote_id=p_quote_id
          and v_design.unit_price=o.unit_price and v_design.quote_v2_price_status='authoritative'
          and s.design_id=v_design_id and s.quote_id=p_quote_id
          and s.provenance_snapshot->>'manualLinePrice'='true'
          and (s.retail_snapshot#>>'{retail,quantity}')::numeric=v_line.quantity)
    then
      v_priced_design_count := v_priced_design_count+1;
      v_event_results := v_event_results || jsonb_build_array(jsonb_build_object(
        'lineItemId',v_line_id,'designId',v_design_id,'snapshotId',v_design.current_v2_snapshot_id,
        'selectionFingerprint',v_design.quote_v2_selection_fingerprint,
        'catalogVersion',v_design.quote_v2_priced_catalog_version,
        'priceStatus','authoritative','selectedDesign',true,'manualPricePreserved',true));
      continue;
    end if;
  ...
  select coalesce(sum(case when designs.options_json->'manual_price_override'='true'::jsonb then 0 else
    coalesce((public.quote_customer_charges(snapshots.retail_snapshot#>'{retail,customerCharges}',lines.quantity)->>'total')::numeric,0) end),0)
    into v_fixed_charges
    from public.sales_quote_line_items lines
    join public.sales_quote_designs designs on designs.id=lines.selected_design_id and designs.line_item_id=lines.id
    join public.sales_quote_v2_price_snapshots snapshots on snapshots.id=designs.current_v2_snapshot_id and snapshots.design_id=designs.id
    where lines.quote_id=p_quote_id and designs.quote_v2_price_status='authoritative';
  v_quote_total := public.quote_customer_adjusted_total(v_quote_total,v_fixed_charges,v_quote.installer_notes);
...
revoke all on function public.save_quote_v2_catalog_pricing_batch(
  uuid, bigint, text, uuid, jsonb
) from public, anon, authenticated;

grant execute on function public.save_quote_v2_catalog_pricing_batch(
  uuid, bigint, text, uuid, jsonb
) to service_role;

comment on function public.save_quote_v2_catalog_pricing_batch(
  uuid, bigint, text, uuid, jsonb
) is
