...
begin
  ...
  -- Apply the same visible contract adjustments after the selected line subtotal.
  begin
    controls := q.installer_notes::jsonb->'__adminControls';
  exception when invalid_text_representation then
    controls := null;
  end;
  if controls->>'showExtras' = 'true' then
    select coalesce(sum((fee->>'amount')::numeric),0) into extras
    from jsonb_array_elements(coalesce(controls->'extraFees','[]'::jsonb)) fee;
  end if;
  if controls->>'showDiscount' = 'true' then
    discount := (total + extras) * coalesce((controls->>'discountPercent')::numeric,0) / 100;
  end if;
  total := total + extras - discount;
  if controls->>'showTax' = 'true' then
    tax := total * coalesce((controls->>'taxPercent')::numeric,0) / 100;
  end if;
