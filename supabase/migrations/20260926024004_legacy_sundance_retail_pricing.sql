-- Extend the existing server-only CAS writer to source-priced Sundance.
-- The historical norman_grid_pricing marker remains compatible with totals/locks.
...
begin
  ...
  if public.read_norman_quote_pricing_state(p_quote_id) is distinct from p_expected_state then
    raise exception 'Quote changed while Catalog pricing was calculated. Reload and retry.' using errcode='40001';
  end if;
  if q.status is distinct from 'draft' or q.quote_v2_backend is true
    ...
    raise exception 'Catalog legacy pricing requires an unsent, unsigned legacy draft.' using errcode='22023';
  end if;
  revision:=coalesce(q.quote_v2_revision,0)+1;
  for r in select value from jsonb_array_elements(p_results) loop
    if (r->>'lineItemId')::uuid=any(seen) then raise exception 'Duplicate Catalog pricing line.' using errcode='22023'; end if;
    ...
    if not found or l.archived_at is not null then raise exception 'Catalog pricing line is not active on this quote.' using errcode='22023'; end if;
    ...
    if not found or d.id is distinct from selected_id or (l.selected_design_id is not null and l.selected_design_id<>d.id) or r->'selectDesign' is distinct from 'true'::jsonb
      or lower(btrim(coalesce(d.supplier,''))) not in ('norman','sundance')
      ...
      raise exception 'Only the selected automatic, unfrozen Catalog design may be priced.' using errcode='22023';
    end if;
    ...
    if jsonb_typeof(selection) is distinct from 'object' or selection->>'manufacturerId' is distinct from lower(btrim(d.supplier))
      ...
      raise exception 'Catalog pricing identity or provenance is invalid.' using errcode='22023';
    end if;
    ...
    if status='authoritative' then
      if jsonb_typeof(snapshot) is distinct from 'object' or snapshot->>'priceStatus' is distinct from status
        ...
        raise exception 'Catalog retail snapshot is incomplete or has a different quantity.' using errcode='22023';
      end if;
      ...
      if unit<0 or once_charge<0 or line_total<0 or round(line_total,2) is distinct from round(unit*l.quantity+once_charge,2) then
        raise exception 'Catalog retail total does not match its unit price and quantity.' using errcode='22023';
      end if;
      ...
      if not unresolved then
        if jsonb_typeof(cost) is distinct from 'object' or jsonb_typeof(cost->'landedCostTotal') is distinct from 'number'
          ...
          raise exception 'Catalog cost must be documented or explicitly unresolved.' using errcode='22023';
        end if;
        ...
        if landed<0 or (cost->>'productCostTotal')::numeric<0 or (cost->>'freightAllocated')::numeric<0
          ...
          raise exception 'Catalog landed cost does not match its components.' using errcode='22023';
        end if;
      ...
      if (snapshot is not null and snapshot<>'null'::jsonb) or (cost is not null and cost<>'null'::jsonb) then
        raise exception 'Blocked Catalog pricing cannot contain a price snapshot.' using errcode='22023';
      end if;
      unit:=0;
      options:=options||jsonb_build_object('authoritative_price_error',
        coalesce(nullif(left(btrim(r->>'staffPricingError'),2000),''),'The selected Catalog grid or option price is unavailable.'));
      blocked:=blocked+1;
    end if;
