...
## Live-found template mount alias repair

On b63930ec,0353D saved solid-frame construction and distinct24×24order-outline measurements, but the missing inside-mount template exception failed to appear. The saved adapter canonicalizes Inside Mount to `inside`; the new rule compared only the display label. The repair accepts exact inside/Inside Mount/IM/I aliases and has an actual exact-interface-adapter regression.16focused tests pass; final universal specialty geometry/account hold prevented sending even before this precise exception was repaired. The draft remains deliberately missing the template reference for post-deployment negative proof.

## YS05 Invisible Tilt curved-section control

Visually reviewed WLP126 and matching BW124/ND134. The two diagrams show the curved horizontal louvers independently operated by a Standard Tilt rod on the back, with the top louver fixed; Invisible Tilt cannot operate the curved section. Added a separate versioned curved-control record and explicit UI choices for the bounded non-Aqua YS05 branch when the main selection is Invisible Tilt. The server requires rear Standard Tilt and fixed-top confirmation. It does not substitute the whole-panel tilt, apply the non-Aqua diagram to Aqua, or infer the construction of other specialty identities. Older records remain readable and older catalog dates preserve behavior; all current specialty geometry/account holds remain.

Thirty focused specialty tests pass, including saved CRM adapter identity, four programs, invalid invisible curved control, fixed-top declaration, persistence and conditional UI. Changed-file typecheck is clean after fixture correction; this worktree still lacks the unrelated database/PDF packages listed in earlier checks. Parent full release gate and production proof remain pending. This closes the depicted YS05/Invisible branch only; remaining specialty tilt/hinge geometry stays in the finite ledger.

## Whole-panel one-louver restrictions

Read WL/BW/ND37 and WLP43: tilt options/rods, split tilt, divider rail, Double Hung and PerfectTilt are unavailable for one-louver panels or sections, and there is no mouse hole. Existing divided/split sections already record counts. Regular and Double-Hung undivided panels now retain an actual whole-panel louver count, with explicit no-mouse-hole confirmation for a count of one; counts are never inferred from height. The server rejects each forbidden combination.

A complete regular, nonmotorized, no-divider/no-split set consisting only of one-louver panels exposes the explicit `No Tilt · Fixed Single Louver` choice. This choice is conditionally accepted server-side; it is not a new universal tilt option. Mixed one-louver/multi-louver configurations remain held for separately modeled controls. Double-Hung single-louver panels remain forbidden. Other track/specialty applications keep their existing holds and are not silently certified by this bounded branch. Older records remain readable and older catalog dates retain behavior.

Forty-two focused panel/assortment/divider tests pass, including all six programs, each prohibition, fixed-control eligibility, mixed counts, actual adapter persistence and UI controls. Changed-file typecheck is clean; baseline missing local DB/PDF dependencies persist. Full release and native production proof are pending. Existing geometry, pricing, and account holds are unchanged.


## Exact regular panel widths and narrow stile joins

Visually confirmed the ≥9-inch boundary in WL12; reread WLP16/BW13/ND14 and all six programs' width tables (WL/BW/ND32, WLP38). Regular L/R/LR/LL/RR/LLRR/LLR/LRR configurations now retain every actual finished panel width. The mixed layouts apply the bifold maximum only to each bifold member, while the neighboring independently hinged panel uses its own program/louver maximum. Nothing divides opening width or copies an old widest-panel summary into missing measurements. Saving the schedule derives its widest-panel summary from the complete measured schedule; the server rejects forged/stale summaries.

Under-nine-inch single panels use Rabbet with documented non-hang-strip frames, Rabbet with a hang strip beside the panel, and Butt with a hang strip behind it. Exact hang-strip position is now saved independently. At nine inches the existing Butt rule applies. The UI narrows choices to the measured branch, and the server independently checks it. T-post/French-door specialized joins remain outside this bounded regular branch. Historical records stay readable; earlier catalog dates retain their previous behavior. Existing account, hardware and final-construction holds are unchanged.

The test matrix covers all six programs, every offered louver size and all eight supported regular layouts at each individual panel's maximum and 1/16 inch beyond, plus the six-inch minimum, exact nine-inch join transition, both hang-strip placements, missing facts, stale summaries, saved adapter inputs and conditional controls. Production save/reopen proof remains pending integration and deployment. This closes basic regular mixed-width and narrow-join selection coverage, not Double Hung, T-post geometry or wide-panel rail fabrication.


## Double Hung two-row construction increment

Reread WL101, WLP117, BW115 and ND117 plus WL32. The current typed Double Hung record explicitly keeps the per-row layout and both complete panel rows, upper left-to-right followed by lower left-to-right. Every panel has independent finished dimensions; a one-row count is rejected. WL/WLP/AquaShield allow only L/R/LR, because their source prohibits bifold. Brightwood and Normandy retain the documented ordinary bifold combinations while rejecting triple/multifold. Mixed hardwood rows apply each panel's proper single/bifold width limit. The widest summary is derived from both rows when saved, not entered independently.

The record preserves center-default versus custom division without guessing the custom datum or converting opening height to panel geometry. A custom division requires its measured location and reference description, and remains subject to the existing final geometry hold. Horizontal T-post presence is explicit, each actual section must be positive and ≤48 inches, and Woodlore panels wider than30 inches with3/3½/4½-inch louvers require the post. Over78-inch upper/lower panels retain the additional-hinge/manufacturer rail-adjustment treatment as a warning; no new surcharge or automatic louver-count change is invented.

For a valid typed two-row schedule, existing sill/bottom-frame site checks apply to the lower row. They are not mislabeled as upper-row sills: the source expressly makes the horizontal post optional for ordinary Double Hung, and upper division support/hinges remain under the final manufacturer geometry hold. Untyped/historical records retain previous interpretation. New row-layout selections start blank measurements, so a prior one-row schedule is not silently reassigned to upper/lower members.

All172 shutter tests pass across17 files, including every program's allowed layouts, two-row count, mixed widths, both48-inch and30-inch boundaries, actual lower-row support, custom-reference persistence, historical parsing and upper/lower controls. Changed-file typecheck is clean with the same unrelated local database/PDF dependency gaps. Integration/deployment and production save/reopen proof remain pending. This closes the bounded two-row identity/basic width/T-post-selection task; actual division/rail/hinge factory schedules and all account pricing remain unresolved.
...
## French-door A–F identities and source restrictions

Implemented bounded typed cutout record from WLP118–119 (diagrams visually checked), BW116–117, ND118–119:28 program/type routes (six each for WoodlorePlus/Brightwood/NormandyPainted/NormandyStained and four A–D for AquaShield). No Woodlore assortment borrowed. Captures exact type, L/R single-panel direction, actual finished panel width/height, top shape and measurement-form reference. A–D require Outside Mount and a current-program L frame excluding quarter-inch light-block variants; save keeps main frame/direction/actual widest width consistent. E/F reject arch/quarter arch and retained A–D frame, derive the source batten width/thickness by louver, reject panel locks, and enforce Butt stiles below9 inches.
