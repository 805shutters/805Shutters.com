...
Quote805-0333 saved, closed and reopened Premier Artisan Weave with Black-Out Chocolate liner, Wide White binding, Single assembly and120-inch custom valance. Changing to Select cleared the prior color and all four accessory fields; Select displayed six liner and twelve binding choices. Select Alisia Antique White with Privacy Gary liner, Wide Pecan binding,2ON1 and119.9375-inch valance then saved, closed and reopened correctly. This Select configuration is the preserved current draft. Both customer delivery actions remained disabled. Exact evidence is in `woven-accessories-live-proof.json`.

The >120-inch rule is tested server-side. The production UI displayed a generic pricing hold during an invalid120.0625-inch check, so no detailed boundary-warning proof is claimed. A saved2ON1 menu choice is not proof of complete order-level shared-headrail modeling; separate component dimensions and pricing remain held.
...
The six previously observed baseline profiles now have an atomic saved options form and independent server validation. See `observed-option-profiles.md` for exact colors, controls,30×60 IM/right/quantity-one scope,26fields/84values, tests, and remaining evidence limitations. This increment is deployed on0fcacb9b and verified live in805-0333: all six profiles and23 selected option values survived close/fullreload/reopen; a31-inch mismatch preserved its record and restoring30 restored its controls. Contract delivery remained blocked. It preserves all pricing holds and does not extrapolate the observed menus to unverified profiles.
