...
- **Wood legacy-label regression passed:** the suspected blanket cut-out guard applies only before September19. Current saved/server repricing accepts valid outer-edge cut-outs with a derived common group even when a legacy common-valance label persists. Two backend regression tests confirm exact group rebuilding, interior-edge rejection and rejection of a forged group record without actual membership. No production rule change is needed; historical restrictions remain intact. This corrects the earlier conditional-code-gap classification.
